...
// zetasMontgomeryNegAVX2 are the negatives of zetasMontgomeryAVX2 (mod q).
// Used in inverse NTT to avoid computing q - zeta at runtime.
var zetasMontgomeryNegAVX2 = [296]fieldElement{}

func init() {
	for i := range zetasMontgomeryNegAVX2 {
		zetasMontgomeryNegAVX2[i] = fieldElement(q) - zetasMontgomeryAVX2[i]
	}
}
...
func inverseButterflyAVX2(even, odd, zetasL, zetasH, outEven, outOdd *avx2.YMM) {
	var t0, t1, qYMM avx2.YMM
	avx2.VMOVDQU_Luint32(&qYMM, []uint32{q, q, q, q, q, q, q, q})

	// outOdd = (Q + even - odd) * zeta
	// Compute this first because outEven can alias even in callers.
	avx2.VPADDD(&t0, &qYMM, even)
	avx2.VPSUBD(&t0, &t0, odd)

	// outEven = even + odd
	avx2.VPADDD(outEven, even, odd)
	// Final reduction: if outEven >= q, subtract q
	avx2.VPCMPGTD(&t1, &qYMM, outEven)
	avx2.VPANDN(&t1, &t1, &qYMM)
	avx2.VPSUBD(outEven, outEven, &t1)
...
func inttLevel0to5AVX2(f []fieldElement, off int) {
	...
	avx2.VPERMQ(&zetasHYMM, &zetasHYMM, 0x1B) // reverse the order of the 4 64-bit words in each 256-bit ymm register
	avx2.VPSRLQ(&zetasYMM, &zetasHYMM, 32)
	inverseButterflyAVX2(&ymm5, &ymm4, &zetasYMM, &zetasHYMM, &ymm5, &ymm4)

	avx2.VMOVDQU_Luint32(&zetasHYMM, []uint32{
		uint32(zetasMontgomeryNegAVX2[296-40-8*off]), uint32(zetasMontgomeryNegAVX2[296-40-8*off+1]), uint32(zetasMontgomeryNegAVX2[296-40-8*off+2]), uint32(zetasMontgomeryNegAVX2[296-40-8*off+3]),
		uint32(zetasMontgomeryNegAVX2[296-40-8*off+4]), uint32(zetasMontgomeryNegAVX2[296-40-8*off+5]), uint32(zetasMontgomeryNegAVX2[296-40-8*off+6]), uint32(zetasMontgomeryNegAVX2[296-40-8*off+7]),
	})
	avx2.VPERMQ(&zetasHYMM, &zetasHYMM, 0x1B)
	avx2.VPSRLQ(&zetasYMM, &zetasHYMM, 32)
	inverseButterflyAVX2(&ymm3, &ymm2, &zetasYMM, &zetasHYMM, &ymm3, &ymm2)

	avx2.VMOVDQU_Luint32(&zetasHYMM, []uint32{
		uint32(zetasMontgomeryNegAVX2[296-72-8*off]), uint32(zetasMontgomeryNegAVX2[296-72-8*off+1]), uint32(zetasMontgomeryNegAVX2[296-72-8*off+2]), uint32(zetasMontgomeryNegAVX2[296-72-8*off+3]),
		uint32(zetasMontgomeryNegAVX2[296-72-8*off+4]), uint32(zetasMontgomeryNegAVX2[296-72-8*off+5]), uint32(zetasMontgomeryNegAVX2[296-72-8*off+6]), uint32(zetasMontgomeryNegAVX2[296-72-8*off+7]),
	})
	avx2.VPERMQ(&zetasHYMM, &zetasHYMM, 0x1B)
	avx2.VPSRLQ(&zetasYMM, &zetasHYMM, 32)
	inverseButterflyAVX2(&ymm1, &ymm0, &zetasYMM, &zetasHYMM, &ymm1, &ymm0)

	avx2.VMOVDQU_Luint32(&zetasHYMM, []uint32{
		uint32(zetasMontgomeryNegAVX2[296-104-8*off]), uint32(zetasMontgomeryNegAVX2[296-104-8*off+1]), uint32(zetasMontgomeryNegAVX2[296-104-8*off+2]), uint32(zetasMontgomeryNegAVX2[296-104-8*off+3]),
		uint32(zetasMontgomeryNegAVX2[296-104-8*off+4]), uint32(zetasMontgomeryNegAVX2[296-104-8*off+5]), uint32(zetasMontgomeryNegAVX2[296-104-8*off+6]), uint32(zetasMontgomeryNegAVX2[296-104-8*off+7]),
	})
	avx2.VPERMQ(&zetasHYMM, &zetasHYMM, 0x1B)
	avx2.VPSRLQ(&zetasYMM, &zetasHYMM, 32)
	...
	avx2.VPERMQ(&zetasHYMM, &zetasHYMM, 0x1B)
	avx2.VPSRLQ(&zetasYMM, &zetasHYMM, 32)
	inverseButterflyAVX2(&ymm5, &ymm3, &zetasYMM, &zetasHYMM, &ymm5, &ymm3)
	inverseButterflyAVX2(&ymm4, &ymm2, &zetasYMM, &zetasHYMM, &ymm4, &ymm2)

	avx2.VMOVDQU_Luint32(&zetasHYMM, []uint32{
		uint32(zetasMontgomeryNegAVX2[168-40-8*off]), uint32(zetasMontgomeryNegAVX2[168-40-8*off+1]), uint32(zetasMontgomeryNegAVX2[168-40-8*off+2]), uint32(zetasMontgomeryNegAVX2[168-40-8*off+3]),
		uint32(zetasMontgomeryNegAVX2[168-40-8*off+4]), uint32(zetasMontgomeryNegAVX2[168-40-8*off+5]), uint32(zetasMontgomeryNegAVX2[168-40-8*off+6]), uint32(zetasMontgomeryNegAVX2[168-40-8*off+7]),
	})
	avx2.VPERMQ(&zetasHYMM, &zetasHYMM, 0x1B)
	avx2.VPSRLQ(&zetasYMM, &zetasHYMM, 32)
	...
	avx2.VPERMQ(&zetasHYMM, &zetasHYMM, 0x1B)
	avx2.VPSRLQ(&zetasYMM, &zetasHYMM, 32)
	inverseButterflyAVX2(&ymm5, &ymm1, &zetasYMM, &zetasHYMM, &ymm5, &ymm1)
	inverseButterflyAVX2(&ymm4, &ymm0, &zetasYMM, &zetasHYMM, &ymm4, &ymm0)
	...
	shuffle2(&ymm5, &ymm4, &ymm6, &ymm4)
	shuffle2(&ymm3, &ymm2, &ymm5, &ymm2)
	shuffle2(&ymm1, &ymm0, &ymm3, &ymm0)
	shuffle2(&ymm8, &ymm7, &ymm1, &ymm7)

	avx2.VMOVDQU_Luint32(&zetasHYMM, []uint32{
		uint32(zetasMontgomeryNegAVX2[72-8-8*off]), uint32(zetasMontgomeryNegAVX2[72-8-8*off+1]), uint32(zetasMontgomeryNegAVX2[72-8-8*off+2]), uint32(zetasMontgomeryNegAVX2[72-8-8*off+3]),
		uint32(zetasMontgomeryNegAVX2[72-8-8*off+4]), uint32(zetasMontgomeryNegAVX2[72-8-8*off+5]), uint32(zetasMontgomeryNegAVX2[72-8-8*off+6]), uint32(zetasMontgomeryNegAVX2[72-8-8*off+7]),
	})
	avx2.VPERMQ(&zetasHYMM, &zetasHYMM, 0x1B)
	avx2.VPSRLQ(&zetasYMM, &zetasHYMM, 32)
	inverseButterflyAVX2(&ymm6, &ymm4, &zetasYMM, &zetasHYMM, &ymm6, &ymm4)
	inverseButterflyAVX2(&ymm5, &ymm2, &zetasYMM, &zetasHYMM, &ymm5, &ymm2)
	...
	//   ymm7 = [28 29 30 31 | 60 61 62 63]
	shuffle4(&ymm6, &ymm5, &ymm8, &ymm5)
	shuffle4(&ymm3, &ymm1, &ymm6, &ymm1)
	shuffle4(&ymm4, &ymm2, &ymm3, &ymm2)
	shuffle4(&ymm0, &ymm7, &ymm4, &ymm7)

	avx2.VMOVDQU_Luint32(&zetasYMM, []uint32{
		uint32(zetasMontgomeryNegAVX2[40-8-8*off]), uint32(zetasMontgomeryNegAVX2[40-8-8*off+1]), uint32(zetasMontgomeryNegAVX2[40-8-8*off+2]), uint32(zetasMontgomeryNegAVX2[40-8-8*off+3]),
		uint32(zetasMontgomeryNegAVX2[40-8-8*off+4]), uint32(zetasMontgomeryNegAVX2[40-8-8*off+5]), uint32(zetasMontgomeryNegAVX2[40-8-8*off+6]), uint32(zetasMontgomeryNegAVX2[40-8-8*off+7]),
	})
	avx2.VPERMQ(&zetasYMM, &zetasYMM, 0x1B)
	inverseButterflyAVX2(&ymm8, &ymm5, &zetasYMM, &zetasYMM, &ymm8, &ymm5)
	inverseButterflyAVX2(&ymm6, &ymm1, &zetasYMM, &zetasYMM, &ymm6, &ymm1)
	...
	//   ymm7 = [56 57 58 59 | 60 61 62 63]
	shuffle8(&ymm8, &ymm6, &ymm0, &ymm6)
	shuffle8(&ymm3, &ymm4, &ymm8, &ymm4)
	shuffle8(&ymm5, &ymm1, &ymm3, &ymm1)
	shuffle8(&ymm2, &ymm7, &ymm5, &ymm7)

	zeta := uint32(zetasMontgomeryAVX2[7-off])
	avx2.VMOVDQU_Luint32(&zetasYMM, []uint32{zeta, zeta, zeta, zeta, zeta, zeta, zeta, zeta})
	avx2.VPSUBD(&zetasYMM, &qYMM, &zetasYMM)
	inverseButterflyAVX2(&ymm0, &ymm6, &zetasYMM, &zetasYMM, &ymm0, &ymm6)
	inverseButterflyAVX2(&ymm8, &ymm4, &zetasYMM, &zetasYMM, &ymm8, &ymm4)
	inverseButterflyAVX2(&ymm3, &ymm1, &zetasYMM, &zetasYMM, &ymm3, &ymm1)
	inverseButterflyAVX2(&ymm5, &ymm7, &zetasYMM, &zetasYMM, &ymm5, &ymm7)

	avx2.VMOVEDQU_Suint32(fUint32[off*64+0:], &ymm0)
	avx2.VMOVEDQU_Suint32(fUint32[off*64+1*8:], &ymm8)
	avx2.VMOVEDQU_Suint32(fUint32[off*64+2*8:], &ymm3)
	avx2.VMOVEDQU_Suint32(fUint32[off*64+3*8:], &ymm5)
	avx2.VMOVEDQU_Suint32(fUint32[off*64+4*8:], &ymm6)
	avx2.VMOVEDQU_Suint32(fUint32[off*64+5*8:], &ymm4)
	avx2.VMOVEDQU_Suint32(fUint32[off*64+6*8:], &ymm1)
	avx2.VMOVEDQU_Suint32(fUint32[off*64+7*8:], &ymm7)
...
func inttLevel6to7AVX2(f []fieldElement, off int) {
	...
	avx2.VMOVDQU_Luint32(&zetasYMM, []uint32{zeta3, zeta3, zeta3, zeta3, zeta3, zeta3, zeta3, zeta3})
	avx2.VPSUBD(&zetasYMM, &qYMM, &zetasYMM)
	inverseButterflyAVX2(&ymm0, &ymm2, &zetasYMM, &zetasYMM, &ymm0, &ymm2)
	inverseButterflyAVX2(&ymm1, &ymm3, &zetasYMM, &zetasYMM, &ymm1, &ymm3)
	...
	avx2.VMOVDQU_Luint32(&zetasYMM, []uint32{zeta1, zeta1, zeta1, zeta1, zeta1, zeta1, zeta1, zeta1})
	avx2.VPSUBD(&zetasYMM, &qYMM, &zetasYMM)
	inverseButterflyAVX2(&ymm0, &ymm4, &zetasYMM, &zetasYMM, &ymm0, &ymm4)
	inverseButterflyAVX2(&ymm1, &ymm5, &zetasYMM, &zetasYMM, &ymm1, &ymm5)
	inverseButterflyAVX2(&ymm2, &ymm6, &zetasYMM, &zetasYMM, &ymm2, &ymm6)
	inverseButterflyAVX2(&ymm3, &ymm7, &zetasYMM, &zetasYMM, &ymm3, &ymm7)

	inverseDegreeMontgomery := uint32(41978) // 41978 = ((256⁻¹ mod q) * (2³² * 2³² mod q)) mod q
	avx2.VMOVDQU_Luint32(&zetasYMM, []uint32{inverseDegreeMontgomery, inverseDegreeMontgomery, inverseDegreeMontgomery, inverseDegreeMontgomery, inverseDegreeMontgomery, inverseDegreeMontgomery, inverseDegreeMontgomery, inverseDegreeMontgomery})
	fieldsMulEvenOddAVX2(&ymm0, &zetasYMM, &zetasYMM, &ymm0)
	fieldsMulEvenOddAVX2(&ymm1, &zetasYMM, &zetasYMM, &ymm1)
	fieldsMulEvenOddAVX2(&ymm2, &zetasYMM, &zetasYMM, &ymm2)
	fieldsMulEvenOddAVX2(&ymm3, &zetasYMM, &zetasYMM, &ymm3)
	fieldsMulEvenOddAVX2(&ymm4, &zetasYMM, &zetasYMM, &ymm4)
	fieldsMulEvenOddAVX2(&ymm5, &zetasYMM, &zetasYMM, &ymm5)
	fieldsMulEvenOddAVX2(&ymm6, &zetasYMM, &zetasYMM, &ymm6)
	fieldsMulEvenOddAVX2(&ymm7, &zetasYMM, &zetasYMM, &ymm7)
...
func inverseNTTAVX2(f nttElement) ringElement {
	for off := 0; off < n/64; off++ {
		inttLevel0to5AVX2(f[:], off)
	}

	for off := 0; off < n/64; off++ {
		inttLevel6to7AVX2(f[:], off)
	}
